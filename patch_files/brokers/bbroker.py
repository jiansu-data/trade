...
class BackBroker(bt.BrokerBase):
    ...
    def _execute(self, order, ago=None, price=None, cash=None, position=None,
        ...
        if order.ordtype == order.Buy:
            order_data_name = order_data_name +".buy"
        elif order.ordtype == order.Sell:
            order_data_name = order_data_name + ".sell"
        comminfo = None
        if  order_data_name  in self.comminfo:
            #print("comminfo ",order_data_name )
            comminfo =  self.comminfo[order_data_name ]
        else:
            comminfo = self.getcommissioninfo(order.data)
